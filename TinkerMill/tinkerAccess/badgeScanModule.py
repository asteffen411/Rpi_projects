    ...
    for port in list(serial.tools.list_ports.comports()):
      ...
      for cfgPortName in C_portName.split():
        if port[0] == cfgPortName:
          serialPort = port[0]
          print ("\nFound a serial port that matches portName %s as defined in scan.cfg\n" % str(cfgPortName) )

    if not serialPort:
      FAILCODE = FAILCODE+"2 Unable to find a serial port that matches description %s as defined in scan.cfg" % C_portName
      return FAILCODE
